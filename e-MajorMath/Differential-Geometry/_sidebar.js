/**
 * Differential Geometry 공통 사이드바
 * 사용법: 각 페이지 <body> 어디에든 <script src="/e-MajorMath/Differential-Geometry/_sidebar.js"></script>
...
(function () {
  ...
  const ITEMS = [
    { href: '/e-MajorMath/Differential-Geometry/1-curves/',      label: 'Curves in the Plane and in Space' },
    { href: '/e-MajorMath/Differential-Geometry/2-curvature/',   label: 'Curvature' },
  ];

  // ── 스타일 주입 (1회) ──
  if (!document.getElementById('_dg_sidebar_css')) {
    const css = `
      .menu-toggle {
  ...
  wrap.innerHTML = `
    <button class="menu-toggle" id="dgMenuToggle" aria-label="메뉴 열기"><span></span></button>
    <div class="sidebar-backdrop" id="dgSidebarBackdrop"></div>
    <aside class="sidebar" id="dgSidebar">
      <a href="${HOME.href}" class="sidebar-home">${HOME.label}</a>
      <div class="sidebar-title">Differential Geometry</div>
  ...
  // ── 토글 ──
  const btn = document.getElementById('dgMenuToggle');
  const sb  = document.getElementById('dgSidebar');
  const bd  = document.getElementById('dgSidebarBackdrop');

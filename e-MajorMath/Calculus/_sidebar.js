/**
 * Calculus 공통 사이드바
 * 사용법: 각 페이지 <body> 어디에든 <script src="/e-MajorMath/Calculus/_sidebar.js"></script>
...
(function () {
  ...
  const ITEMS = [
    { href: '/e-MajorMath/Calculus/5-epsilon-delta/index.html',          label: 'ε-δ 정의 시각화' },
    { href: '/e-MajorMath/Calculus/20-fubini/multiple-integrals.html',   label: '다중적분' },
    { href: '/e-MajorMath/Calculus/30-greens-theorem/',                    label: '그린 정리' },
    { href: '/e-MajorMath/Calculus/40-surface-integrals/',               label: '곡면 적분' },
    { href: '/e-MajorMath/Calculus/50-divergence-theorem/',              label: '발산 정리' },
    { href: '/e-MajorMath/Calculus/60-stokes-theorem/',                  label: '스토크스 정리' },
  ];

  // ── 스타일 주입 (1회) ──
  if (!document.getElementById('_calc_sidebar_css')) {
    const css = `
      .menu-toggle {
  ...
  wrap.innerHTML = `
    <button class="menu-toggle" id="calcMenuToggle" aria-label="메뉴 열기"><span></span></button>
    <div class="sidebar-backdrop" id="calcSidebarBackdrop"></div>
    <aside class="sidebar" id="calcSidebar">
      <a href="${HOME.href}" class="sidebar-home">${HOME.label}</a>
      <div class="sidebar-title">Calculus</div>
  ...
  // ── 토글 ──
  const btn = document.getElementById('calcMenuToggle');
  const sb  = document.getElementById('calcSidebar');
  const bd  = document.getElementById('calcSidebarBackdrop');

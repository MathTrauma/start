...
import { XPolygon, XRightAngle, XDimension, XText, XPoint } from '../../lib/x_object.js';
...
async function loadHtml(file, targetId) {
    try {
        const url = `./problems-mid3/040/${file}`;
...
async function loadSolutionHtml() {
    try {
        const url = `./problems-mid3/040/solution.html`;
        const res = await fetch(url);
        if (res.ok) {
...
function createSketch(pid) {
    return (p) => {
        ...
        p.setup = function () {
            ...
            // 기본 점: 직각삼각형 (∠B=90°), AB=2√2, BC=1, AC=3, sinA=1/3
            A = p.createVector(0, 0);
            B = p.createVector(2 * Math.SQRT2, 0);
            C = p.createVector(2 * Math.SQRT2, 1);
            ...
            animator.registerPhase('problem1', [
                ...
                { id: 'rightABC', object: new XRightAngle(p, A, B, C, 16, { pixel: true }), animate: { mode: 'draw', duration: 0.5 } },
                { delay: 0.2 },
                {
                    group: [
                        { id: 'dimAC', object: new XDimension(p, A, C, '3', { offset: 10 }), animate: { mode: 'draw', duration: 0.7 } },
                        { id: 'dimBC', object: new XDimension(p, B, C, '1', { offset: -10 }), animate: { mode: 'draw', duration: 0.7 } }
                    ],
                    parallel: true
                }
            ]);
            ...
            animator.registerPhase('solution1', [
                {
                    group: [
                        { id: 'text1', object: new XText(p, [20, 25], '\\overline{AB} = \\sqrt{3^2 - 1^2} = \\sqrt{8}', { useTex: true, fontSize: 18, screenCoord: true, textAlign: p.LEFT }), animate: { mode: 'draw', duration: 1.2 } },
                        { id: 'dimAB', object: new XDimension(p, A, B, '2\\sqrt2', { offset: -10, useTex: true }), animate: { mode: 'draw', duration: 0.7 } }
                    ],
                    parallel: true
                }
            ]);

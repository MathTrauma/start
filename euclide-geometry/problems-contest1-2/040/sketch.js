...
import { XPolygon, XSegment, XPoint, XArc, XDimension, XObject, XText } from '../../lib/x_object.js';
...
const sketch = (p) => {
    ...
    p.setup = function () {
        ...
        // XText 사용 (screenCoord: true)
        const textCeva = new XText(p, [baseX, baseY], '체바정리', { fontSize: 18, screenCoord: true });

        const formula1 = new XText(p, [baseX, baseY + LINE_HEIGHT], 
            '\\frac{\\overline{AE}}{\\overline{EB}} \\cdot \\frac{\\overline{BM}}{\\overline{MC}} \\cdot \\frac{\\overline{CF}}{\\overline{FA}}=1', 
            { fontSize: 14, useTex: true, screenCoord: true }
        );

        const formula2 = new XText(p, [baseX, baseY + LINE_HEIGHT * 2], 
            '\\frac{\\overline{AE}}{\\overline{EB}} = \\frac{\\overline{AF}}{\\overline{FC}} \\Longrightarrow EF \\parallel BC', 
            { fontSize: 14, useTex: true, screenCoord: true }
        );
